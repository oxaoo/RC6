...
private:
	...
	void decryptBlock(const BYTE* pIn, BYTE* pOut); //дешифрование блоков текста.
	void decrypt(string pathToEncodeFile); //дешифрование текста.

public:
	CRC6Decoder(string pathToKeyFile, string pathToTextFile, string pathToDecodeFile);
	CRC6Decoder();
	~CRC6Decoder();
};